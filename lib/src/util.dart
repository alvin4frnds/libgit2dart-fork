...
String getLibName() {
  // On Android the plugin packages libgit2.so into jniLibs/<abi>/ and the
  // OS linker resolves it by the SONAME the linker emitted — which our
  // NDK build wrote as "libgit2.so" (not the versioned Linux form).
  // Short-circuit before the versioned name+path heuristics below.
...
DynamicLibrary loadLibrary(String name) {
  // Android: skip the desktop path-resolution heuristics. _resolveLibPath
  // calls `Process.runSync('dart', …)` which throws ProcessException on
  // Android (no dart binary in the app sandbox). The Android dynamic
  // linker resolves plain `libgit2.so` against the APK's jniLibs dir
  // automatically.
  if (Platform.isAndroid) {
